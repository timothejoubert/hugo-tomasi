...
export default (Vue as VueConstructor<Component>).extend({
    ...
    computed: {
        rootClass(): (string | undefined | false)[] {
            return [
                this.$style.root,
                this.itemsMounted && this.$style['root--mounted'],
    ...
    watch: {
        value(splashState: SplashScreenState) {
            if (splashState === 'beforeEnter' && this.itemsMounted) this.onBeforeEnter()
    ...
    mounted() {
        ...
        if (headerOverTitle) {
            ...
            this.itemsMounted = true
            if (this.value === 'beforeEnter') this.onBeforeEnter()
        }
    },
    methods: {
        onBeforeEnter() {
